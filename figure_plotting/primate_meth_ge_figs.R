...
#LOAD THE LOGOS
source('metadata/species_logos/primate/logo_source.R')
logo_corner = 0.95
logo_scale = 0.2
...
for (n in dat_names){
  ...
  logo = logo_list[[n]]
  gdf = gene_lvl_list[[n]]
  ghist = plot_bimodal_logo(gdf, logo, logo_scale, xcorner=0.45, BREAKS=SET_BREAKS)
...
ry = 1/30
rx = 1/15
ghists = assemble_row_panels(gene_hist_list, 1, '%methylation', 'gene count', ry, rx)
ghists
...
for (n in dat_names){
  ...
  logo = logo_list[[n]]
  gdf = gene_lvl_list[[n]]
  ghist = plot_bimodal_primate(gdf, logo, logo_scale, xcorner=0.45)
  gene_hist_list[[n]] = ghist
}
...
plot_lvl_lvl = function(n, mlvl_list){
  ...
  merge_df = inner_join(m_df, r_df,
                        by='name')
  plt = plot_scatter_pearsonCor_annotated(merge_df, 'fracMeth', 'lfpkm', 'GBM', 'expression level', ALPHA=0.1) +
...
ry = 1/30
rx = 1/15
glvl_ge = assemble_row_panels(glvls_plt_list, 1, '% methylation', 'expression level', ry, rx)


# PROMOTER METHYLATION HISTOGRAMS -----------------------------------------

promoter_hist_list = list()
for (n in dat_names){
  print(n)
  logo = logo_list[[n]]
  pdf = promoter_lvl_list[[n]]
  phist = plot_bimodal_primate(pdf, logo, logo_scale, xcorner=0.45)
  promoter_hist_list[[n]] = phist
}
...
ry = 1/30
rx = 1/15
phists = assemble_row_panels(promoter_hist_list, 1, '%methylation', 'gene count', ry, rx)
phists



# PROMOTER LEVEL AND GE LEVEL --------------------------------------------
...
ry = 1/30
rx = 1/15
plvl_ge = assemble_row_panels(plvls_plt_list, 1, '% methylation', 'expression level', ry, rx)



# PLOT DIFFERENTIAL EXPRESSION BETWEEN GBM GROUPS -----------------------------

ll=load('figure_plotting/primate_deseq_res_list.Rdata') #use processing_scripts/deseq_treatment_effects.R to build this
ll

plot_lvl_change = function(n, mlvl_list, res_list){
  logo = logo_list[[n]]
  m_df = mlvl_list[[n]]
  r_df = res_list[[n]] %>% 
    data.frame() %>% 
  ...
  merge_df = inner_join(m_df, r_df,
                        by='name')
  plt = plot_scatter_pearsonCor_annotated(merge_df, 'fracMeth', 'abs_diff', 'GBM', 'expression difference',
                                          ALPHA=0.1,
                                          ylim = c(0,4)) +
    geom_smooth(method='lm', se=FALSE) +
    theme(plot.title = element_text(hjust=0.5)) +
...
for (n in dat_names){
  g_change_plt_list[[n]] = plot_lvl_change(n, gene_lvl_list, liver_list)
}
...
ry = 1/30
rx = 1/15
gdiff_ge = assemble_row_panels(g_change_plt_list, 1, '% methylation', 'expression difference', ry, rx)

# PLOT DIFFERENTIAL EXPRESSION BETWEEN PROMOTER METHYLATION GROUPS -----------------------------

#run the function
p_change_plt_list = list()
for (n in dat_names){
  p_change_plt_list[[n]] = plot_lvl_change(n, promoter_lvl_list, liver_list)
}
...
ry = 1/30
rx = 1/15
pdiff_ge = assemble_row_panels(p_change_plt_list, 1, '% methylation', 'expression difference', ry, rx)
...
#reformat ge results
ge_change_list = map(liver_list, function(x) data.frame(x) %>% 
                       rownames_to_column('name') %>% 
                       dplyr::select(name, log2FoldChange, pvalue, padj))

#load the gbm response results from methylkit
gene_mkit_list = list()
for (n in dat_names){
...
ry = 1/30
rx = 1/15
gbm_response = assemble_row_panels(gbm_response_plotlist, 1, 'GBM change', 'expression change', ry, rx)


# GE CHANGE AND PROMOTER CHANGE --------------------------------------------------
...
ry = 1/30
rx = 1/15
promoter_response = assemble_row_panels(promoter_response_plotlist, 1, 'promoter methylation change', 'expression change', ry, rx)

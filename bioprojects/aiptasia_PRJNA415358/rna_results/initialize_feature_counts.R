...
rnaDir='bioprojects/aiptasia_PRJNA415358/rna_results/'
...
coldata=tibble(
  run=sample,
  treat=factor(if_else(grepl('A', sample),
                       'treated',
                       'control')),
  treatInfo = if_else(grepl('A', sample),
                      'aposymbiotic',
                      'symbiotic')) %>% 
  data.frame()
rownames(coldata) = coldata$run
coldata
write_tsv(coldata, path='bioprojects/aiptasia_PRJNA415358/rna_treat_table.txt')
